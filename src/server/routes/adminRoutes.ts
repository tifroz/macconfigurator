...
import type { AppConfig, ValidationError } from "../../core/types.js";

export function createAdminRoutes(service: ConfigStorageService, logger: any): Router {
  const router = Router();

  // Helper to handle Effect results
  async function handleEffect<T>(effect: Effect.Effect<T, Error | ValidationError[]>, res: any, successStatus = 200) {
