...
// 173 Lines by Claude Sonnet
// Comprehensive test to reproduce ConfigValidationError with service layer integration - converted to TypeScript
...
import { validateConfig } from "../core/validation/schemaValidator.js";
...
async function testDirectValidation(): Promise<ValidationResult> {
  ...
  const configResult = await Effect.runPromise(validateConfig(testConfig, testSchema));
...
async function testServiceLayerValidation(): Promise<AppConfig | null> {
...
async function testInvalidSchemaValidation(): Promise<void> {
  ...
  try {
    ...
    if (error._tag === "ConfigValidationError") {
      ...
      error.errors.forEach((err: any) => console.log(`  - ${err.field}: ${err.message}`));
    } else {
...
async function testConfigAgainstInvalidSchema(): Promise<void> {
...
async function testComplexScenarios(): Promise<void> {
  ...
  const program = Effect.gen(function* () {
    ...
    const emptySchemaApp: AppConfig = {
    ...
    try {
      yield* service.createApplication(emptySchemaApp);
    ...
    const noMetaSchemaApp: AppConfig = {
    ...
    try {
      yield* service.createApplication(noMetaSchemaApp);
